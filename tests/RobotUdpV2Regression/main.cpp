...
protocol::Bytes ControlPacket(protocol::Source source,
                              ...
                              const protocol::ControlPayload &payload = SampleControl())
{
...
void TestCodec()
{
    ...
    auto bytes = ControlPacket(protocol::Source::Master,
...
void TestFullStatus()
{
    ...
    Expect(protocol::encodeFullStatus(header, status, bytes, &error) &&
               protocol::decodeFullStatus(bytes.data(),
                                          bytes.size(),
                                          decodedHeader,
                                          decoded,
                                          &error) &&
...
double PF(std::size_t offset)
{
    return static_cast<double>(offset % 251) + 0.5;
}
    ...
    for (const auto &candidate : candidates) {
        ...
        while (file.get(c)) {
            ...
            if (hi < 0) {
                ...
                bytes.push_back(static_cast<std::uint8_t>((hi << 4) | digit));
                hi = -1;
            }
        }
        usedPath = candidate;
...
protocol::ControlPayload GoldenControlPayload()
{
    ...
    for (std::size_t i = 0; i < payload.ercp_6d.size(); ++i)
        payload.ercp_6d[i] = i + 10.5;
...
protocol::FullStatusPayload GoldenStatusPayload()
{
    ...
    for (std::size_t j = 0; j < 36; ++j)
        s.beckhoff_common.values[j] = PF(120 + 8 * j);
    ...
    for (std::size_t j = 0; j < 10; ++j)
        r0.command.values[j] = PF(640 + 8 * j);
    ...
    for (std::size_t j = 0; j < 10; ++j)
        r1.command.values[j] = PF(768 + 8 * j);
                      ...
                      const char *what)
{
    if (actual.size() != golden.size()) {
        ++failures;
        std::cerr << "FAIL: " << what << " size " << actual.size() << " != golden " << golden.size()
                  << '\n';
        return;
    }
    for (std::size_t i = 0; i < actual.size(); ++i) {
        if (actual[i] != golden[i]) {
            ++failures;
            std::cerr << "FAIL: " << what << " first byte mismatch at offset " << i << " (actual 0x"
                      << std::hex << int(actual[i]) << " golden 0x" << int(golden[i]) << std::dec
                      << ")\n";
            return;
        }
    }
...
void TestGoldenStatusFixture()
{
    ...
    const auto recordEqual = [](const protocol::AppliedCommandRecord &a,
        ...
        return a.command.values == b.command.values && a.command.switches == b.command.switches &&
               ...
               a.received_unix_ns == b.received_unix_ns && a.applied_unix_ns == b.applied_unix_ns &&
               a.ads_error == b.ads_error;
    };
    ...
    Expect(
        decoded.ads_diagnostics.snapshot_sequence == fixture.ads_diagnostics.snapshot_sequence &&
            ...
            decoded.ads_diagnostics.common_ads_error == fixture.ads_diagnostics.common_ads_error &&
