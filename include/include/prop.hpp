...
template <types T> struct prop_tag : public _tag_ {
...
template <typename T> class prop_value : public _prop_ {
public:
    using value_type = T;
...
public:
    prop_item(const this_is_private,
              prop_namespace &parent,
              const std::string &name,
              types type,
              prop_ptr def);

    template <typename... Args> static std::shared_ptr<prop_item> create(Args &&...args)
    {
...
public:
    ...
    template <typename T> void set(const std::string &field, const T &val)
    {
        auto p = find(field);
        if (p) {
...
template <> struct prop_tag<types::_bool> : public _tag_ {
    YAML::Node get() const override;
};

template <> struct prop_tag<types::_int> : public _tag_ {
    std::string unit;
...
template <> struct prop_tag<types::_float> : public _tag_ {
    std::string unit;
...
template <> struct prop_tag<types::_string> : public _tag_ {
...
template <> struct prop_tag<types::_enum> : public _tag_ {
...
template <> struct prop_tag<types::_vector> : public _tag_ {
    std::string unit;
    size_t size;
