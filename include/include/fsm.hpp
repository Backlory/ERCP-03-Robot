...
template <typename action,
          typename obj,
          typename event,
          bool f1 = is_invocable<action>::value,
          bool f2 = is_invocable<action, obj>::value,
...
{
    return _invoke_helper<action, obj, event>::invoke(std::forward<action>(f),
                                                      std::forward<obj>(a),
                                                      std::forward<event>(b));
}
...
template <typename...> struct _transition_table {};

////////////////////////////////////////////////////////////////
template <class...> struct _transition_table_concat;

// For end of the concating.
template <> struct _transition_table_concat<> {
    using type = _transition_table<>;
};

template <class T, class... Types> struct _transition_table_concat<T, _transition_table<Types...>> {
    using type = _transition_table<T, Types...>;
};

////////////////////////////////////////////////////////////////
template <template <typename> typename _Condition, typename...> struct _transition_filter;

///< For those no valid filtered items.
template <template <typename> typename _Condition> struct _transition_filter<_Condition> {
    using type = _transition_table<>;
};
...
/**
     ...
     */
template <typename State> class fsm {
public:
...
protected:
        ...
        : m_state(init_state)
    {
        static_assert(std::is_trivially_copyable<State>::value,
                      "`State` should be trivially copyable.");
    }
...
public:
    template <typename Object,
              ...
              typename Event,
              typename State to,
              void (Object::*action)() = nullptr,
              bool (Object::*guard)() const = nullptr>
...
protected:
    template <class Event> State no_transition(const Event &event)
    {
...
protected:
    template <class Event, typename...> struct do_transition_filter;

    ///< For those no valid filtered items.
    template <typename Event> struct do_transition_filter<Event, detail::_transition_table<>> {
    ...
    struct do_transition_filter<Event, detail::_transition_table<Items...>> {
        template <typename Item> using predicate = std::is_same<typename Item::event_t, Event>;
        using type = typename detail::_transition_filter<predicate, Items...>::type;
    };

protected:
    template <typename Object, typename Event, typename...> struct do_transition;
...
protected:
    template <typename Object, typename...> struct test_transition;

    ///< For those unimplemented transition.
    template <typename Object> struct test_transition<Object, detail::_transition_table<>> {
