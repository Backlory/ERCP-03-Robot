...
template <typename... Args> class _TasksBase : public _TaskBase<Args...> {
public:
    ...
    {
        m_tasks.emplace_back(std::make_shared<task_t>(task_t::m_name + "/" + name,
                                                      std::move(fn),
                                                      skipable,
                                                      verbose));
    }
...
template <typename... Args> class SequentialTasks : public _TasksBase<Args...> {
public:
...
template <typename... Args> class ParallelTasks : public _TasksBase<Args...> {
public:
    using base_t = _TasksBase<Args...>;
...
template <typename... Args> using tasks_ptr = std::shared_ptr<_TaskBase<Args...>>;
template <typename... Args> using const_tasks_ptr = std::shared_ptr<const _TaskBase<Args...>>;

template <typename... Args> using seque_ptr = std::shared_ptr<SequentialTasks<Args...>>;

template <typename... Args> using paral_ptr = std::shared_ptr<ParallelTasks<Args...>>;
