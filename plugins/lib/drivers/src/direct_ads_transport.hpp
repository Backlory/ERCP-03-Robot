...
public:
    ...
    std::uint32_t WriteControl(std::uint16_t adsState, std::uint16_t deviceState,
        std::uint32_t length, const void *data);
    std::uint32_t Read(std::uint32_t indexGroup, std::uint32_t indexOffset,
        std::uint32_t length, void *data);
    std::uint32_t Write(std::uint32_t indexGroup, std::uint32_t indexOffset,
        std::uint32_t length, const void *data);
    std::uint32_t ReadWrite(std::uint32_t indexGroup, std::uint32_t indexOffset,
