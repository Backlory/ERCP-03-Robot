...
public:
   ...
   @param numAxes Number of axes for a multi-axis EtherCAT drive
   ...
   */
   const Error *Init( Amp &amp, uint16 slot, int numAxes )
   {
      ...
      const Error *err = SetType( 1 ); // YTOM-20211012: solve PDO message block bus problem.
      ...
      if( err )
         cml.Debug( "Amp %d Status PDO Init error %s\n", amp.GetNodeID(), err->toString() );
      else
         cml.Debug( "Amp %d Status PDO Init finished successfully\n", amp.GetNodeID() );
 ...
 * A separate thread is necessary, because EtherCAT drives need to send
 * SDOs when handling PVT status updates, and SDOs can't be used from
 * within the context of the network read thread.
 */
#define MAX_PENDING_PVT_STATUS     50
class PvtThread: public Thread
{
   Mutex mtx;
   Semaphore sem;
   int next, ct;
   ...
   struct 
   {
      uint32 ampRef;
      uint32 pvtStat;
   } pending[MAX_PENDING_PVT_STATUS];
...
public:
   ...
   const Error *init( int pri=9 )
   {
      MutexLocker ml( mtx );
      if( running ) return 0;

      const Error *err = setPriority( pri );
      if( err )
      {
         cml.Error( "Error setting priority of PVT thread: %s\n", err->toString() );
         return err;
      }
      ...
      if( err )
         cml.Error( "Error starting PVT thread: %s\n", err->toString() );
      else
         running = true;
   ...
   void Add( uint32 ampRef, uint32 stat )
   {
      MutexLocker ml( mtx );
      if( ct >= MAX_PENDING_PVT_STATUS )
      {
         cml.Error( "Unable to handle PVT status update, too many updates pending\n" );
         return;
      }

      // Increase the reference count for the amp to prevent it from being reused
      // before I have a chance to handle 
      if( !RefObj::GrabRef( ampRef ) )
      {
         cml.Error( "Amp reference (0x%08x) passed to PVT status update no longer valid\n", ampRef );
         return;
      }

      int ndx = (next + ct) % MAX_PENDING_PVT_STATUS;
      ct++;
      pending[ndx].ampRef = ampRef;
      pending[ndx].pvtStat = stat;
      sem.Put();
   }

   uint32 Get( uint32 *stat )
   {
      MutexLocker ml( mtx );
      if( !ct ) return 0;
      ct--;
      int n = next++;
      if( next >= MAX_PENDING_PVT_STATUS ) 
         next = 0;
      *stat = pending[n].pvtStat;
      return pending[n].ampRef;
   }
   ...
   void run( void )
   {
      while( 1 )
      {
         ...
         if( err )
         {
            cml.Warn( "PVT status thread error getting semaphore: %s \n", err->toString() );
            sleep( 10 );
            continue;
         }

         uint32 ampRef, pvtStat;
         while( (ampRef=Get(&pvtStat)) != 0 )
         {
            RefObjLocker<Amp> ampPtr( ampRef );
            if( ampPtr )
               ampPtr->PvtStatusUpdate( pvtStat );
            else
               cml.Error( "Amp (ref 0x%08x) removed while updating PVT status\n", ampRef );

            RefObj::ReleaseRef( ampRef );
         }
      }
   }
};
static PvtThread pvtThread;

/**
Transmit PDO used to send out PVT buffer status updates
*/
class TPDO_PvtStat: public TPDO
{
   uint32 ampRef[MAX_AXIS];
...
public:
   ...
   const Error *Init( Amp &amp, uint16 slot, uint16 numAxes )
   {
      cml.Debug( "Amp %d Initting PVT status PDO for %d axes\n", amp.GetNodeID(), numAxes );

      CML_ASSERT( numAxes <= MAX_AXIS );
      if( numAxes > MAX_AXIS ) return &AmpError::BadAxis;
      axisCt = numAxes;

      nodeID = amp.GetNodeID();
      ...
      if( !err ) pvtThread.init();

      if( err )
         cml.Debug( "Amp %d PVT status PDO init ended with error %s\n", nodeID, err->toString() );
      else
         cml.Debug( "Amp %d PVT status PDO init ended successfully\n", nodeID );
      return err;
   }
   ...
   {
      for( int i=0; i<axisCt; i++ )
         pvtThread.Add( ampRef[i], stat[i].Read() );
   }
...
@param delayMS   The status will only be requested if we haven't requested or
                 received a status update from the amp in this much time (milliseconds).
 */
/***************************************************************************/
const Error *Amp::RequestStatUpdt( uint32 delayMS )
{
   ...
   if( now - statPDO->GetLastRxTime() < delayMS )
      return 0;

   if( now - lastStatRqTime < delayMS )
      return 0;
...
public:
   ...
   bool HandleRxMsg( void ){ return false; }
};
