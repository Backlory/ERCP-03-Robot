...
static protocol::v2::ControlPayload
applied_control(const beckhoff_follow_cmd &follow_cmd,
                const device::beckhoff::GoldDiscreteCommand &discrete)
{
    ...
    applied.values[controlIndex(ControlValueIndex::CutterFeed)] = follow_cmd.vel_cutter_feed;
    applied.values[controlIndex(ControlValueIndex::CutterSwing)] = follow_cmd.vel_cutter_rot;
    applied.values[controlIndex(ControlValueIndex::CutterBend)] = follow_cmd.vel_cutter_bend;
    applied.values[controlIndex(ControlValueIndex::GuideWireFeed)] = follow_cmd.vel_wire_feed;
    applied.switches =
        (follow_cmd.home_rotate ? 1u << 0 : 0u) | (follow_cmd.home_bend_lr ? 1u << 1 : 0u) |
        (follow_cmd.home_bend_ud ? 1u << 2 : 0u) | (follow_cmd.switch_water ? 1u << 3 : 0u) |
        (follow_cmd.switch_gas ? 1u << 4 : 0u) | (follow_cmd.switch_suct ? 1u << 5 : 0u);
    ...
    std::copy(std::begin(discrete.handle_6d),
              std::end(discrete.handle_6d),
    ...
    applied.inject_enables =
        (discrete.inject_enable[0] ? 1u << 0 : 0u) | (discrete.inject_enable[1] ? 1u << 1 : 0u);
    return applied;
}
...
void YunSBot::_base::ControlRunnable2(double t)
{
    ...
    if (automaticMode && m_master_priority.load()) {
        ...
        if (parent.master.GetCommand(probeCommand, probeMetadata, kMasterPriorityWindowSeconds)) {
            masterOverride = true;
    ...
    const auto selectedSource =
        useMaster ? protocol::v2::Source::Master : protocol::v2::Source::Cloud;
    ...
    const bool ercpAllowed = fresh && robot.BeckhoffIsERCPOnline() && robot.BeckhoffIsERCPReady();
    ...
    const auto result = !succeeded ? protocol::v2::ApplyResult::Failed
    ...
    m_applied_commands
        .MarkAttempt(appliedCommand, metadata, result, adsError, appliedAt, succeeded);
}

// 建立跟随命令信息
void YunSBot::_base::build_follow_cmd(const protocol::v2::ControlPayload &cmd,
                                      beckhoff_follow_cmd &follow_cmd)
{
    ...
    follow_cmd.vel_rotate = clamp_control(cmd.values[controlIndex(ControlValueIndex::ScopeRotate)]);
